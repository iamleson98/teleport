...
func TestValidateHealthCheckConfig(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			requireErr: errContains("invalid spec.db_labels: selector *:<val> is not supported"),
		},
		{
		...
		},
		{
			...
			in: &healthcheckconfigv1.HealthCheckConfig{
				...
				Spec: &healthcheckconfigv1.HealthCheckConfigSpec{
					...
					Timeout:  durationpb.New(time.Second),
					Interval: durationpb.New(360 * time.Second),
				},
			},
			requireErr: errContains("spec.interval must not be greater than 5m0s"),
		},
	}
